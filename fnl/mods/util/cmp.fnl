(fn config []
  (let [{: setup
           ...
           select-next-cmp (fn [fallback] (if (visible)
                                              (select_next_item)
                                              (expand_or_jumpable)
                                              (expand_or_jump)
                                              (fallback)))
    ...
    (lazy_load)
    (setup {
           :snippet {:expand #(lsp_expand $1.body)}
           :formatting {:format (cmp_format
                                  {:maxwidth 50
                                  :before (fn [{:source {: name}} item]
                                            (tset item :menu (. menu name))
                                            item)})}
           :mapping (mapping.preset.insert
                      {:<C-u> (mapping.scroll_docs -4)
                      :<C-d> (mapping.scroll_docs 4)
                      :<C-Space> {:i toggle-cmp :c toggle-cmp}
